...
class Demodulator:
    ...
    def decode_stream(self, m_history):
        substance = []
        for i in range(1, len(m_history)):
            # On extrait la variation (la "vibration" du savoir)
            delta = abs(m_history[i] - m_history[i-1])
            
            # On mappe le delta sur l'alphabet (Modulus 26)
            if delta > 0.05:
                char_idx = int(delta * 10) % 26
                substance.append(self.alphabet[char_idx])
            else:
                substance.append(" ") # Silence dissipatif

        text = "".join(substance)
        # NETTOYAGE : Supprime les répétitions et crée des mots
        text = re.sub(r'(.)\1{2,}', r'\1', text) # pas de 'aaaaa'
        text = re.sub(r'(\w{5})', r'\1 ', text)  # force des mots de 5 lettres
